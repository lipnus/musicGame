...
public class HomeManager : MonoBehaviour {
	...
	IEnumerator Scenario(float delayTime) { 
		...
		GameObject.Find("textManager").GetComponent<TextManager>().showText("잠이 오지 않아..");
		...
		switchSound.Play();
		GameObject.Find("textManager").GetComponent<TextManager>().showText("공허한 마음을 달래줄 것은 오직 음악..");
		yield return new WaitForSeconds(1);
